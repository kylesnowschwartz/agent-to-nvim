...
user retype their edits as a comment. Handing the draft to nvim lets them edit it
directly, and returns the text they actually want.

Use this the moment a draft exists that is headed somewhere outside the
conversation. Don't ask permission first - hand it over, then act on what comes
back.
...
- **A path to an existing file** - hand that file over directly. It's edited in
...
For a draft that isn't already a file, write it to the drafts directory under
the user's home:
...
Spell the home directory out. A file tool takes an absolute path and doesn't
expand `~`, so a literal `~/...` lands in a directory called `~` beside wherever
you're working. The home directory is the leading part of the paths you already
...
Write tool; don't try to pipe multiline text into the command. Don't run `mktemp`
...
costs nothing: the harness re-invokes you when the command exits, whether that's
...
- **Exit 20 isn't a failure to retry.** The user closed the draft on purpose.
  Reopening it overrides a decision they just made. Say the draft was discarded and
  ask what they want instead.
- **Exit 30 isn't a timeout to give up on, and it isn't one to poll on either.**
...
- **A scratch draft** (written to the drafts directory) comes back whole on
  stdout. Use exactly that text. Don't merge it with the original draft or
...
- **A real file edited in place** prints nothing on stdout - the file already
  ...
  complete record of what happened. **Don't read the file back to "check", and
  don't open any truncated tool-output file the harness points at** - everything
...
A line the user starts with `>>` is a note to you rather than draft text. There
are two kinds, and the number of angle brackets says the scope:
...
- `>>>` is about the whole draft - tone, structure, length, whether to send it at
  all. It has no anchor in the text, so don't hunt for "the line it means".
...
The quoted lines say where a note was written, and nothing about how far it
reaches. Take the scope from what the note says and use the lines only to place
it. A note at the very top or bottom shows only the one line it has beside it.
...
- **A question, doubt, or open point** ("is this the right channel?", "check that
  ...
  rewrite happens. Answer it, or do the investigating it asks for, in the
  conversation, then wait for the user's take before revising and handing the
  draft back. Rewriting past an unanswered question guesses at the answer and
  hands the user another edit to redo.
...
question costs one reply, and a wrong rewrite costs a redo.

Exit 0 with `draft text unchanged, 2 notes` means they left the wording alone and
told you something instead. There is still work to do - don't read it as
approval.

## Read what changed

On exit 0, stderr carries the change itself, so there's no need to work it out by
...
- **Anything inside `[- -]` is gone on purpose.** Don't restore it, and don't
  reword it back in on the grounds that it read better.
- **A `~` line changed only the marked words.** The rest of that line was left
  alone, so don't treat the whole line as rewritten.

When the edit was substantive, it's worth saying so to the user - name what they
changed rather than replaying the whole text back at them. Say it in plain words;
`[-` and `{+` are markers for reading the report, and repeating them at somebody
who just made the edit by hand helps nobody.
