...
The quoted lines say where a note was written, not how far it reaches. Plenty of
notes are about the whole draft, so take the scope from what the note says and use
the lines only to place it.

A blank line means the next quote is from somewhere else in the draft. Two `>>`
lines in a row are two notes about the same place.
